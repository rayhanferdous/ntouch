            ...
            data.forEach((menu) => {
                ...
                menu.categories.forEach((category) => {
                    ...
                    category.items.forEach((item) => {
                        ...
                        // Check how many prices there are
                        let priceOptions;
                        if (item.prices.length === 1) {
                            const priceValue = (item.prices[0].value / 100).toFixed(2);
                            priceOptions = `<span>$${priceValue}</span>`; // Only show the price value
                        } else {
                            priceOptions = item.prices.map(price => {
                                const priceValue = (price.value / 100).toFixed(2);
                                return `<span>${price.name}: $${priceValue}</span>`;
                        ...
                        detailsDiv.innerHTML = `
                            ...
                            <div class="text-green-600 font-semibold flex flex-col gap-1 w-fit">${priceOptions}</div>
                            </div>
